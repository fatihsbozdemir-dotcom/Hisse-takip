...
TELEGRAM_TOKEN = os.environ.get("TELEGRAM_TOKEN", "8550118582:AAHftKsl1xCuHvGccq7oPN-QcYULJ5_UVHw")
...
def tg_foto(buf, caption):
    try:
        ...
        print(f"[FOTO] HATA: {e}"); return False
...
def grafik(df, hisse, tepeler, dipler, tip, sev):
    ...
    ekler = ([mpf.make_addplot([s]*len(son), color="#FF4444", linestyle="--", width=1.2) for s,_ in tepeler[:3]] +
    ...
    stil = mpf.make_mpf_style(marketcolors=mc, facecolor="#131722", figcolor="#131722",
                               edgecolor="#2A2E39", gridcolor="#2A2E39", gridstyle="--",
                               y_on_right=True,
    ...
    fig, _ = mpf.plot(son, type="candle", addplot=ekler,
                      title=f"\n{hisse}  {fiyat:.2f} TL  {'🟢 ALIŞ' if tip=='ALIS' else '🔴 SATIŞ'} BÖLGESİ",
...
def analiz_et():
    ...
    # 0. Token kontrolü
    print("ADIM 0 — Token kontrol")
    try:
        r = requests.get(f"https://api.telegram.org/bot{TELEGRAM_TOKEN}/getMe", timeout=10)
        print(f"  getMe: {r.status_code} | {r.text[:250]}")
        if not r.ok:
            print("!!! TOKEN GECERSIZ"); return
        bot = r.json()["result"]["username"]
        print(f"  Bot: @{bot}")
    except Exception as e:
        print(f"  HATA: {e}"); return

    # 1. Telegram mesaj testi
    print(f"ADIM 1 — Mesaj testi | Chat ID: [{CHAT_ID}]")
    if not tg_yaz(f"🤖 *Hisse Tarama Botu başladı* (@{bot})\nHisseler taranıyor..."):
        print(f"!!! MESAJ GITMEDI — Chat ID yanlis: [{CHAT_ID}]")
        print("    Kanal icin: @kanal_adi ya da -100xxxxx formatinda olmali")
        return

    # 2. Hisse listesi
    print("ADIM 2 — Hisse listesi")
    try:
        ...
        tg_yaz(f"📋 *{len(hisseler)} hisse taranıyor...*")
    except Exception as e:
        print(f"  HATA: {e}")
        tg_yaz(f"❌ Sheets okunamadı: `{e}`"); return

    # 3. Tarama
    print("ADIM 3 — Tarama başlıyor")
    bulunan = 0
    for i, hisse in enumerate(hisseler):
        try:
            df = yf.Ticker(f"{hisse}.IS").history(period="1y", interval="1d")
            if len(df) < 60:
                print(f"  [{i+1}/{len(hisseler)}] {hisse}: veri yok ({len(df)})"); continue
            ...
            if tip:
                e = "🟢" if tip == "ALIS" else "🔴"
                print(f"  [{i+1}] {hisse}: {e} {tip}! {fiyat:.2f} -> sev={sev:.2f} guc={guc}x")
                ...
                caption = (f"{e} *{'ALIŞ' if tip=='ALIS' else 'SATIŞ'} BÖLGESİ*\n"
                           f"📊 *{hisse}*\n"
                           f"💰 `{fiyat:.2f} TL` ({degis:+.2f}%)\n"
                           f"🎯 Seviye: `{sev:.2f}` Güç:{guc}x Uzaklık:%{uzak:.1f}\n"
                           f"🟢 Destek: {d_str}\n"
                           f"🔴 Direnç: {t_str}")
                tg_foto(buf, caption); bulunan += 1
            else:
                print(f"  [{i+1}/{len(hisseler)}] {hisse}: sinyal yok ({fiyat:.2f})")
        except Exception as e:
            print(f"  [{i+1}] {hisse}: HATA — {e}"); continue

    print(f"\n{'='*60}\nTamamlandı: {bulunan} sinyal")
    tg_yaz("📊 *Tarama tamamlandı.*\nŞu an sinyal yok." if bulunan == 0
           else f"✅ *Tarama tamamlandı.* {bulunan} sinyal gönderildi.")
